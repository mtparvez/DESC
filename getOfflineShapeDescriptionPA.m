...
function [descShape, descCode, segStartPos, segEndPos, segLenghts, segXY] = ...
    getOfflineShapeDescriptionPA(strokeX, strokeY, segLens, repParam)
    isDebug = 0;
    ...
    if ~isempty(segLens)
        ...
        [domX, domY, ~] = getPolygonalAppoximation(strokeX, strokeY, 1);
    end
...
%     strokeX(domPos) = domX;
    ...
    for idom = 1 : nd -1
        ...
        if isRotationInv == 1
            ...
            if idom > 1
                ...
                [midShapeClass, midShapeDesc] = isDirectionChanged(xCord, yCord, diffAng);                
            end
    ...
    if isRotationInv == 1
        segCode = ['L', 'V', 'C', 's', 'l', 'r'];
        segDesc = [{'straight_line'},{'cw_curve'},{'ccw_curve'},{'same_dir'},{'bent_left'},{'bent_right'}];
