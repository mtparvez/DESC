function [shapeClass, shapeDesc] = getShapeInfoVeryRelaxed(x, y, angle)
    isCut = 0;    
...
%         else
            if pos == 0 %x(2) >= (x(1)+x(3))/2 %angle >= 90 && angle < 180
            shapeClass = 'A';
...
%         else
            if pos == 1%x(2) >= (x(1)+x(3))/2  %angle >= 90 && angle < 180
            shapeClass = 'H';
